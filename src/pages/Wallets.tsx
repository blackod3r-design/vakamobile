  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className={`text-3xl font-bold mb-1 tracking-tight ${isDark ? 'text-white' : 'text-gray-900'}`}>Billeteras</h1>
          <p className={isDark ? "text-gray-400" : "text-muted-foreground"}>Gestiona tu efectivo y cuentas manuales</p>
        </div>
            ...
            <div className="space-y-4 pt-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <Input
                    ...
                    className={`w-16 h-10 cursor-pointer p-1 ${isDark ? 'bg-zinc-900 border-zinc-700' : ''}`}
                  />
                  <Input
                    type="text"
                    value={newWallet.color}
                    onChange={(e) => setNewWallet({ ...newWallet, color: e.target.value })}
                    className={`flex-1 ${isDark ? 'bg-zinc-900 border-zinc-700' : ''}`}
                    placeholder="#3b82f6"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <Select value={newWallet.currency} onValueChange={(val) => setNewWallet({ ...newWallet, currency: val as '$' | 'S/' })}>
                  <SelectTrigger id="wallet-currency" className={isDark ? 'bg-zinc-900 border-zinc-700' : ''}>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
      ...
      {/* --- TOTALES (Estilo Apple/Black Glass) --- */}
      {wallets.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
          {/* Card Soles */}
          <div className={`relative w-full overflow-hidden rounded-[32px] border p-8 shadow-2xl backdrop-blur-3xl transition-transform hover:-translate-y-1 flex flex-col items-center justify-center text-center
              ${isDark 
                  ? 'bg-[#181818] border-[#27272a] shadow-black hover:border-zinc-700' 
                  : 'bg-white/70 border-white/60 shadow-blue-900/5 ring-1 ring-white/70'
              }
          `}>
            ...
            <p className={`text-6xl font-light tracking-tight tabular-nums drop-shadow-sm leading-none mb-2 ${isDark ? 'text-white' : 'text-gray-900'}`}>
      ...
      {/* --- GRID DE BILLETERAS (Estilo Apple/Black Glass Personalizable) --- */}
      {wallets.length === 0 ? (
        <Card className={`p-12 text-center border-dashed border-2 rounded-[32px] ${isDark ? 'bg-[#181818]/50 border-[#27272a]' : 'bg-muted/20 border-muted-foreground/20'}`}>
          <Wallet className="w-16 h-16 mx-auto mb-4 text-muted-foreground/50" />
          <h3 className={`text-xl font-semibold mb-2 ${isDark ? 'text-gray-200' : 'text-gray-700'}`}>No hay billeteras</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          {wallets.map((wallet) => {
            ...
            const shadowStyle = isDark ? { boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)' } : { boxShadow: `0 25px 50px -12px ${wallet.color}30` };
            ...
            return (
              <div
                ...
                style={{ ...shadowStyle }}
                className={`group relative w-full overflow-hidden rounded-[32px] border p-8 backdrop-blur-3xl transition-all duration-300 hover:-translate-y-2 cursor-pointer flex flex-col items-center text-center
                    ${isDark 
                        ? 'bg-[#181818] border-[#27272a] hover:border-zinc-700' 
              ...
              >
                ...
                <div className="mb-4 flex flex-col items-center">
                    ...
                    <h3 className={`text-2xl font-semibold tracking-tight relative z-10 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                        {wallet.name}
                    </h3>
                </div>
                ...
                <div className="relative z-10 flex items-baseline justify-center gap-1.5 mb-2">
                  <span className={`text-3xl font-light ${isDark ? 'text-zinc-500' : 'text-gray-400'}`}>{wallet.currency}</span>
                ...
                <p className={`text-xs font-medium relative z-10 ${isDark ? 'text-zinc-500' : 'text-gray-400'}`}>
      ...
      <Dialog open={!!selectedWalletId} onOpenChange={(open) => !open && setSelectedWalletId(null)}>
        <DialogContent className={`max-w-4xl max-h-[85vh] h-[85vh] p-0 overflow-hidden flex flex-col backdrop-blur-xl ${isDark ? 'bg-[#0f0f0f]/95 border-zinc-800 text-white' : 'bg-zinc-50/95'}`}>
          {(() => {
                  ...
                  <div className="grid md:grid-cols-2 h-full">
                    ...
                    <div className={`p-8 overflow-y-auto space-y-8 h-full border-r ${isDark ? 'bg-black/20 border-zinc-800' : 'bg-white/40 border-gray-200/50'}`}>
                        ...
                        <div className="text-center space-y-2 py-8">
                            ...
                            <div className="flex items-baseline justify-center gap-1">
                                ...
                                <span className={`text-7xl font-bold tracking-tighter ${isDark ? 'text-white' : 'text-gray-900'}`}>{wallet.saldo.toLocaleString()}</span>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            ...
                            <Button 
                                size="lg" 
                                variant="outline" 
                                className={`w-full h-14 text-lg gap-3 border-red-200 text-red-600 hover:bg-red-50 hover:text-red-700 hover:border-red-300 ${isDark ? 'bg-transparent border-red-900/50 text-red-500 hover:bg-red-900/20' : ''}`}
                        ...
                        <div className="pt-8 flex justify-center">
                            <Button 
                                variant="ghost" 
                                className="text-gray-400 hover:text-red-500 hover:bg-red-500/10 gap-2"
                    ...
                    <div className={`flex flex-col h-full p-8 overflow-hidden ${isDark ? 'bg-[#181818]' : 'bg-white'}`}>
                        <div className="flex items-center justify-between mb-6 shrink-0">
                            <h3 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>Movimientos</h3>
                        ...
                        {wallet.movimientos.length === 0 ? (
                            ...
                            <div className="flex-1 overflow-y-auto pr-2 space-y-3 scrollbar-hide">
                                {wallet.movimientos.slice().reverse().map((mov) => (
                                    <div key={mov.id} className={`flex justify-between items-center p-4 rounded-xl border transition-colors group ${isDark ? 'bg-[#202020] border-zinc-800 hover:bg-[#252525]' : 'bg-gray-50 border-gray-100 hover:bg-gray-100'}`}>
                                        <div className="flex items-center gap-4 min-w-0">
                                            ...
                                            <div className="min-w-0">
                                                <p className={`font-semibold truncate ${isDark ? 'text-white' : 'text-gray-800'}`}>{mov.descripcion || 'Sin descripción'}</p>
            ...
            <div>
              ...
              <div className="relative">
                <Input
                  ...
                  type="number"
                  placeholder="0.00"
                  className={`pl-8 text-lg font-bold ${isDark ? 'bg-zinc-900 border-zinc-700' : ''}`}
