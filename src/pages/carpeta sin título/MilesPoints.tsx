  ...
  return (
    <div className="p-8 animate-fade-in" style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Helvetica Neue', sans-serif" }}>
      ...
      <div className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-gray-900">Millas y Puntos</h1>
          <p className="text-muted-foreground">Gestiona tus programas de lealtad</p>
        </div>
          ...
          <DialogContent>
            ...
            <form onSubmit={handleAddProgram} className="space-y-4 pt-4">
              <div className="space-y-2">
                ...
                <Input id="name" placeholder="Ej: LATAM Pass" value={name} onChange={(e) => setName(e.target.value)} required />
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <Input id="points" type="number" placeholder="0" value={points} onChange={(e) => setPoints(e.target.value)} required />
              </div>
      ...
      {/* --- GRID DE PROGRAMAS (Estilo Apple Glass) --- */}
      {milesPoints.programs.length === 0 ? (
        <div className="text-center py-20 bg-muted/20 rounded-[32px] border border-dashed border-muted-foreground/20">
          ...
          <h3 className="text-xl font-semibold text-gray-700">Sin programas registrados</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {milesPoints.programs.map((program) => {
            ...
            // Configuración de colores dinámicos según el tipo (Plane=Blue, Shopping=Purple, Gift=Pink)
            const colorMap = {
                blue: {
                    badge: 'bg-blue-100/50 text-blue-700 border-blue-200/50',
                    shadow: 'shadow-blue-900/5 hover:shadow-blue-900/10',
                    glow: 'bg-blue-500/20'
                },
                purple: {
                    badge: 'bg-purple-100/50 text-purple-700 border-purple-200/50',
                    shadow: 'shadow-purple-900/5 hover:shadow-purple-900/10',
                    glow: 'bg-purple-500/20'
                },
                pink: {
                    badge: 'bg-pink-100/50 text-pink-700 border-pink-200/50',
                    shadow: 'shadow-pink-900/5 hover:shadow-pink-900/10',
                    glow: 'bg-pink-500/20'
                }
            };

            // @ts-ignore
            const theme = colorMap[config.color] || colorMap.blue;
            ...
            return (
              ...
              >
                ...
                {/* Botón Edit (Solo visible en modo edición) */}
                {editMode && (
                   <button 
                     onClick={(e) => { e.stopPropagation(); setCustomizeProgramId(program.id); setIsCustomizeOpen(true); }}
                     className="absolute top-6 left-6 p-2 rounded-full bg-black/5 hover:bg-black/10 transition-colors text-gray-500 hover:text-black z-20"
                   >
                ...
                <h3 className="mb-2 text-2xl font-bold tracking-tight text-gray-800 relative z-10">
                  {program.name}
                </h3>
                ...
                <div className="relative z-10 mb-8">
                  ...
                  <p className="text-sm text-gray-400 font-medium mt-1">puntos acumulados</p>
                </div>

                {/* 6. Footer Discreto */}
                <div className="relative z-10 w-full border-t border-gray-200/50 pt-4 mt-auto">
      ...
      {/* --- MODAL DETALLE DEL PROGRAMA (Fixed Height) --- */}
      <Dialog open={!!selectedProgramId} onOpenChange={(open) => !open && setSelectedProgramId(null)}>
        <DialogContent className="max-w-4xl h-[80vh] p-0 overflow-hidden flex flex-col bg-zinc-50/95 backdrop-blur-xl">
          {(() => {
              ...
              <div className="flex flex-col h-full w-full">
                ...
                <DialogHeader className="px-8 py-6 border-b border-gray-200/50 bg-white/50 shrink-0">
                  <DialogTitle className="text-2xl font-bold text-gray-800 flex items-center gap-3">
                     <config.Icon className="w-6 h-6 text-zinc-900" />
                    ...
                    <div className="p-8 overflow-y-auto space-y-8 bg-white/40 h-full border-r border-gray-200/50">
                        ...
                        <div className="grid grid-cols-1 gap-3">
                            ...
                            <Button 
                                size="lg" 
                                variant="outline" 
                                className="w-full h-12 gap-2 border-gray-300 text-gray-700 hover:bg-gray-100"
                        ...
                        <div className="pt-4 border-t border-gray-200/50">
                            {!selectedProgram.imageUrl ? (
                                <div className="relative group cursor-pointer border-2 border-dashed border-gray-300 rounded-xl p-4 text-center hover:border-zinc-400 hover:bg-gray-100 transition-all">
                        ...
                        {selectedProgram.transactions.length === 0 ? (
                            ...
                            <div className="flex-1 overflow-y-auto pr-2 space-y-3 scrollbar-hide">
                                {selectedProgram.transactions.slice().reverse().map((t) => (
                                    <div key={t.id} className="flex justify-between items-center p-4 rounded-xl bg-gray-50 border border-gray-100 hover:bg-gray-100 transition-colors">
        ...
        <DialogContent>
            ...
            <form onSubmit={handleAddTransaction} className="space-y-4 pt-4">
                <div className="space-y-2">
                    ...
                    <Input type="number" value={transactionAmount} onChange={(e) => setTransactionAmount(e.target.value)} autoFocus />
                </div>
                <div className="space-y-2">
                    <Label>Descripción</Label>
                    <Input value={transactionDescription} onChange={(e) => setTransactionDescription(e.target.value)} placeholder={transactionType === 'add' ? 'Vuelo a Madrid' : 'Canje de producto'} />
                </div>
