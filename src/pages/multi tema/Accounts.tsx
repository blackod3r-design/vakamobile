...
const Accounts = () => {
  ...
  const getPageBackground = () => {
    ...
    return 'bg-white text-gray-900'; // Light sólido
  };
  ...
  // ----------------------------------------------------------------------
  // 7. RENDER UI (Con Lógica de Tema Inyectada)
  // ----------------------------------------------------------------------
  return (
    // APLICAMOS EL FONDO DINÁMICO
    <div className={`p-8 animate-fade-in min-h-screen w-full transition-colors duration-500 ${pageBgClass}`} style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Helvetica Neue', sans-serif" }}>
      
      <div className="flex items-center justify-between mb-6">
        <div>
          {/* Título adaptable */}
          <h1 className={`text-3xl font-bold mb-2 tracking-tight ${theme && theme.includes('dark') ? 'text-white' : 'text-gray-900'}`}>
            Cuentas
          </h1>
        </div>
      ...
      {accounts.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
          {/* Card Soles */}
          <div className={`relative w-full overflow-hidden rounded-[32px] border p-8 shadow-2xl transition-transform hover:-translate-y-1 flex flex-col items-center justify-center text-center ${
              theme === 'dark' || theme === 'dark-glass' || theme?.includes('dark') 
              ? 'bg-[#181818] border-[#27272a] shadow-black hover:border-zinc-600' 
              : theme === 'glass' 
                ? 'bg-white/70 border-white/60 shadow-blue-900/5 backdrop-blur-3xl' 
                : 'bg-white border-gray-100 shadow-xl shadow-blue-900/10'
          }`}>
            <div className={`pointer-events-none absolute inset-x-0 top-0 h-1/2 bg-gradient-to-b ${theme === 'dark' ? 'from-white/5' : 'from-white/50'} to-transparent opacity-50`}></div>
            
            <p className={`text-6xl font-light tracking-tight tabular-nums drop-shadow-sm leading-none mb-2 ${theme && theme.includes('dark') ? 'text-white' : 'text-gray-900'}`}>
          ...
          <div className={`relative w-full overflow-hidden rounded-[32px] border p-8 shadow-2xl transition-transform hover:-translate-y-1 flex flex-col items-center justify-center text-center ${
              ...
              ? 'bg-[#181818] border-[#27272a] shadow-black hover:border-zinc-600' 
              : theme === 'glass' 
                ? 'bg-white/70 border-white/60 shadow-emerald-900/5 backdrop-blur-3xl' 
                : 'bg-white border-gray-100 shadow-xl shadow-emerald-900/10'
          }`}>
             <div className={`pointer-events-none absolute inset-x-0 top-0 h-1/2 bg-gradient-to-b ${theme === 'dark' ? 'from-white/5' : 'from-white/50'} to-transparent opacity-50`}></div>
      ...
      {/* --- GRID DE CUENTAS (Estilos Dinámicos Inyectados) --- */}
      {accounts.length === 0 ? (
        <div className="text-center py-12">
          <Wallet className="w-16 h-16 text-muted-foreground mx-auto mb-4" />
          <h3 className={`text-xl font-semibold mb-2 ${theme && theme.includes('dark') ? 'text-white' : ''}`}>No tienes cuentas todavía</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          {accounts.map((account) => {
            ...
            if (theme === 'dark' || theme === 'dark-glass' || theme.includes('dark')) {
                ...
                shadowStyle = 'shadow-black';
            } 
            // Override Glass Mode
            else if (theme === 'glass') {
                cardContainerClass = 'bg-white/70 border-white/40 backdrop-blur-3xl ring-1 ring-white/70 hover:shadow-lg shadow-2xl';
                // (Mismos textos que light, colores de badge base)
            } 
            // Override Light Mode
            else {
                cardContainerClass = 'bg-white border-gray-100 shadow-xl shadow-blue-900/10 hover:shadow-blue-900/20';
            ...
            return (
              <div
                ...
                className={`group relative w-full overflow-hidden rounded-[32px] border p-8 transition-all duration-300 hover:-translate-y-2 cursor-pointer flex flex-col items-center text-center ${cardContainerClass} ${shadowStyle}`}
              >
                <div className={`pointer-events-none absolute inset-x-0 top-0 h-1/2 bg-gradient-to-b ${theme === 'dark' ? 'from-white/5' : 'from-white/50'} to-transparent opacity-50`}></div>

                {isDPF && (
                   <div className="absolute -right-20 -top-20 h-64 w-64 animate-pulse rounded-full bg-purple-500/10 blur-3xl mix-blend-multiply transition-all duration-700 group-hover:bg-purple-500/20"></div>
                )}
                ...
                <div className={`relative z-10 flex items-baseline justify-center gap-1.5 ${isDPF ? 'mb-10' : 'mb-6'}`}>
                  <span className={`text-3xl font-light ${textSubClass}`}>{account.currency}</span>
                ...
                {isDPF && (
                  <div className="relative z-10 w-full">
                    <div className={`mb-2 flex justify-center gap-2 text-xs font-medium ${textMutedClass}`}>
                      <span>Progreso:</span>
